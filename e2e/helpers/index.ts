...
export class DefaultTestEnv implements TestEnv {
  ...
  constructor(options: Options = {}) {
    this.options = {
      ...
      notificationTimeout: options.notificationTimeout ?? 500,
  ...
  async approveForEnable(page?: Page): Promise<void> {
    ...
    const button = await notificationPage.getByRole('button', { name: /connect/i });
    await button.click();
  }
  ...
  getInjectedCkb(page?: Page): InjectedCkb<RpcMethods> {
    ...
    const ckb: InjectedCkb = {
      ...
      request: async (payload) => {
        ...
        if (this.options.autoApprove && payload.method === 'wallet_enable') {
